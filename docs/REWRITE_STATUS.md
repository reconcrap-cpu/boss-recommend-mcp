...
Last updated: 2026-05-05 10:55 Asia/Shanghai
...
Latest chat CV acquisition correction: dev-ready plus targeted live-smoke verified on 2026-05-05 10:50 Asia/Shanghai. The failed npm chat run `mcp_chat_morwsmrl_9zefu23r` proved Boss chat Network payloads were profile-only, not full CVs, and showed the old chat image fallback was capturing the surrounding page background. Chat now keeps Network as the primary path, but when Network has already produced profile-only payloads the DOM fallback uses a short probe (`3500ms` first profile-only case, `1500ms` when the run is already in image mode) instead of the previous full `120000ms` wait before image fallback. Chat image fallback now targets the popup wrapper before content nodes, uses clipped modal capture (`captureViewport=false`), and uses zero padding so LLM images contain the resume modal rather than the chat page. Live smoke command `node scripts/live-chat-domain-smoke.js --no-navigate --detail-source image --candidate-index 0 --resume-dom-timeout-ms 3500 --ready-timeout-ms 90000 --card-timeout-ms 90000 --max-image-pages 3 --save-payload .live-artifacts\chat-modal-clip-nopad-smoke-20260505-105027.json --save-image .live-artifacts\chat-modal-clip-nopad-smoke-20260505-105027.jpg` passed against Chrome `127.0.0.1:9222`, blocked `Runtime.evaluate`, captured three scroll images with `capture_viewport=false`, first clip `{x:337.5,y:0,width:765,height:773}`, and no `Runtime.*`. Regression checks passed: `npm run test:chat-run-service`, `npm run test:chat-domain`, `npm run test:chat-mcp`, `node --check scripts\live-chat-domain-smoke.js`, and `npm run scan:runtime:package:strict`. This is not yet an npm-published fix.
...
## 2026-05-05 11:22 Asia/Shanghai - Chat Image Fallback Modal Crop And Stop Boundary

- Current phase: Phase 10 hardening / image-fallback reliability and speed.
- Active workstream: shared `core/capture` full-CV scroll screenshots, chat online-resume image fallback, and report auditability.
- Status: live-verified for a one-candidate chat image fallback smoke; dev-ready for the same shared capture primitive to be applied to recommend/search stop boundaries if later Boss pages show recommendation-card tail noise there.
- Change implemented: `captureScrolledNodeScreenshots` now supports CDP-only DOM-anchor scrolling with input fallback, per-step and total capture timeouts, duplicate-tail recovery, JPEG resize/compression, LLM image composition, and dynamic stop-boundary detection. The stop-boundary probe uses `DOM.querySelectorAll`, `DOM.getOuterHTML`, and `DOM.getBoxModel`; it does not use page JS.
- Chat integration: chat online-resume fallback captures the resume modal clip (`captureViewport=false`, `padding=0`) instead of the full viewport, scrolls the modal content, and stops when Boss mounts non-CV tail sections such as other-candidate recommendations / analysis / privacy footer markers. The report summary now preserves scroll-anchor and stop-boundary metadata inside `image_evidence`.
- Live test date/time: 2026-05-05 11:21-11:22 Asia/Shanghai.
- Chrome target used: `127.0.0.1:9222`, `https://www.zhipin.com/web/chat/index`.
- Exact live command: `node scripts/live-chat-image-quality.js --slow-live --start-from all --target-count 1 --detail-source image --max-candidates 1 --max-image-pages 8 --delay-ms 1200 --llm-timeout-ms 480000 --no-request-cv --save-report C:\Users\yaolin\Desktop\screen_csv\chat_image_quality_stopboundary_final_20260505-112152.report.json --out-dir C:\Users\yaolin\Desktop\screen_csv\chat_image_quality_stopboundary_final_20260505-112152`.
- Observed result: PASS; run `chat_image_quality_mos2b0jo_ti3ctsgf`, `processed=1`, `screened=1`, `detail_opened=1`, forced image fallback, `capture_count=7`, `screenshot_count=5`, `unique_screenshot_count=5`, `dropped_duplicate_count=1`, `llm_screenshot_count=2`, optimized screenshot bytes `476883` from original `1024033`, LLM composite bytes `451161`, `Runtime.*=0`.
- Artifacts: `C:\Users\yaolin\Desktop\screen_csv\chat_image_quality_stopboundary_final_20260505-112152.report.json`, `C:\Users\yaolin\Desktop\screen_csv\chat_image_quality_stopboundary_final_20260505-112152\chat_image_quality_mos2b0jo_ti3ctsgf\chat-candidate-001-page-01.jpg` through `chat-candidate-001-page-05.jpg`, and `chat-candidate-001-llm-01.jpg` / `chat-candidate-001-llm-02.jpg`.
- Visual review: final LLM composites preserve the full visible CV scroll sequence and no longer include the unrelated candidate recommendation-card tail seen in the earlier chat artifact. The final page can still include Boss analysis/privacy footer text if it appears before the recommendation-card boundary; tighten the boundary further only after confirming it does not cut real CV content.
...
## 2026-07-16 19:04 Asia/Shanghai - Release 2.1.22 And Installed-Runtime Acceptance

- Published `@reconcrap/boss-recommend-mcp@2.1.22` (`latest=2.1.22`), installed it globally, updated Trae/Trae-CN MCP routes to the exact `2.1.22` pin, mirrored the packaged skills, and passed `doctor --agent trae-cn --page-scope recommend --port 9222` with `RECOMMEND_READY` and no login requirement.
- Activity input is free-form at the MCP boundary and normalizes to the closest canonical option. Exact values, Chinese/English aliases, relative durations, qualitative intent, and one-edit typos are supported; conflicting, ambiguous, or unintelligible input falls back to `不限`. Verified examples: `past week -> 本周活跃`, `10 days -> 本周活跃`, `very active -> 刚刚活跃`, `不限或今日活跃 -> 不限`, and unknown text `-> 不限`.
- Installed-runtime live command used `--cooldown-ms 0` and applied current-city-only `true`, activity `今日活跃`, school tags `985+211`, and degrees `本科+硕士+博士` with `post_action=none`.
- Sticky verification passed for every group; detected city was `杭州`; the page was left applied. Visual review confirmed the checked `仅推荐期望城市为本城市的牛人` checkbox, highlighted `今日活跃`, `985`, `211`, `本科`, `硕士`, and `博士`, plus the settled header `仅看杭州` and `筛选:6`.
- The 2,125-call live method log recorded `Runtime.*=0` and script injection `=0`; the guarded client also blocks `Page.addScriptToEvaluateOnNewDocument` before transport. Artifacts are under `.live-artifacts\recommend-filter-apply-2.1.22\`.
