...
async function waitForMcpChatRun(workspaceRoot, runId, predicate, timeoutMs = 5000) {
  const started = Date.now();
  while (Date.now() - started <= timeoutMs) {
    const payload = await callTool(workspaceRoot, TOOL_BOSS_CHAT_GET_RUN, { run_id: runId }, 900);
    if (predicate(payload?.run)) return payload.run;
    await sleepMs(50);
  }
  throw new Error(`Timed out waiting for MCP chat run ${runId}`);
}
...
async function testBossChatMcpToolsShouldValidateAndRoute() {
  await withBossChatWorkspace(async (workspaceRoot) => {
    ...
    let prepareConnectorClosed = 0;
    setChatMcpConnectorForTests(async () => ({
      client: { guarded: true },
      target: {
        id: "fake-chat-prepare-target",
    ...
    assert.equal(prepared.status, "NEED_INPUT");
    assert.deepEqual(prepared.missing_fields, ["job", "start_from", "target_count", "criteria"]);
    assert.equal(prepared.runtime_evaluate_used, false);
    assert.equal(prepared.job_options.length, 2);
    assert.equal(prepareConnectorClosed, 1);
    ...
    assert.equal(needInput.job_options.length, 0);
    ...
    let healthConnectorClosed = false;
    setChatMcpConnectorForTests(async () => ({
      client: { guarded: true },
      target: {
        id: "fake-chat-health-target",
        url: "https://www.zhipin.com/web/chat/index",
        type: "page"
      },
      methodLog: [
        { method: "DOM.getDocument", at: new Date().toISOString() },
        { method: "DOM.querySelectorAll", at: new Date().toISOString() },
        { method: "Accessibility.getFullAXTree", at: new Date().toISOString() }
      ],
      navigation: {
        navigated: false,
        url: "https://www.zhipin.com/web/chat/index"
      ...
      async close() {
        healthConnectorClosed = true;
      }
    }));
    const health = await callTool(workspaceRoot, TOOL_BOSS_CHAT_HEALTH_CHECK, {}, 13);
    assert.equal(health.status, "OK");
    assert.equal(health.mode, "cdp-only");
    assert.equal(health.runtime_evaluate_used, false);
    assert.equal(health.method_summary["DOM.getDocument"], 1);
    assert.equal(health.cli_path, null);
    assert.equal(healthConnectorClosed, true);
    resetChatMcpStateForTests();
    ...
    try {
      ...
      assert.equal(started.run.context.greeting_text, "您好，方便投递一份简历吗？");
      assert.equal(started.run.context.job, "算法工程师");
      assert.equal(started.run.context.start_from, "unread");
      assert.equal(started.run.context.target_count, 2);

      const runningRun = await waitForMcpChatRun(
        workspaceRoot,
        started.run_id,
        (run) => run?.progress?.processed >= 1
      );
      assert.equal(runningRun.state, "running");

      const pausedRequested = await callTool(workspaceRoot, TOOL_BOSS_CHAT_PAUSE_RUN, {
        run_id: started.run_id,
        profile: "default"
      }, 16);
      assert.equal(pausedRequested.status, "PAUSE_REQUESTED");
      const paused = await waitForMcpChatRun(workspaceRoot, started.run_id, (run) => run?.state === "paused");
      ...
      await waitForMcpChatRun(
        workspaceRoot,
        started.run_id,
        (run) => (run?.progress?.processed || 0) > paused.progress.processed
      );
      ...
      const final = await waitForMcpChatRun(workspaceRoot, started.run_id, (run) => run?.state === "canceled");
      ...
      assert.equal(startedAll.status, "ACCEPTED");
      assert.equal(startedAll.run.context.target_count, "all");
      assert.equal(startedAll.run.context.max_candidates, 2);
      ...
      assert.equal(startedCamelCase.status, "ACCEPTED");
      assert.equal(startedCamelCase.run.context.target_count, "all");

      await waitForMcpChatRun(workspaceRoot, startedAll.run_id, (run) => run?.state === "completed");
      await waitForMcpChatRun(workspaceRoot, startedCamelCase.run_id, (run) => run?.state === "completed");
    } finally {
      resetChatMcpStateForTests();
    }
  });
  ...
  await withBossChatWorkspace(async (workspaceRoot) => {
    try {
      ...
      setChatMcpJobReaderForTests(async () => ({
        ...
        selected_label: "全部职位",
        job_options: [
          { label: "算法工程师", value: "算法工程师", index: 0, active: true }
        ]
      }));

      const healthLogs = await captureConsoleLogs(async () => {
        await cliTestables.runBossChatCliCommand("health-check", {
          "workspace-root": workspaceRoot,
          "slow-live": true,
          port: "9666"
        });
      });
      const health = JSON.parse(healthLogs[0]);
      assert.equal(health.status, "OK");
      assert.equal(health.cdp_only, true);
      assert.equal(health.runtime_evaluate_used, false);
      assert.equal(health.method_summary["DOM.enable"], 1);
      ...
      assert.equal(payload.error.code, "CHAT_CLI_ASYNC_UNSUPPORTED_CDP_ONLY");
      assert.equal(payload.runtime_evaluate_used, false);

      const allInput = cliTestables.buildBossChatCliInput({
        job: "算法工程师",
        "start-from": "all",
        criteria: "全部候选人都过一遍",
        targetCount: "全部候选人",
      ...
      assert.equal(allInput.target_count, "全部候选人");
      assert.equal(allInput.detail_limit, 0);
      assert.equal(allInput.delay_ms, 1200);
      assert.equal(allInput.max_candidates, 3);
      assert.equal(allInput.slow_live, true);
    } finally {
      resetChatMcpStateForTests();
    }
  });
