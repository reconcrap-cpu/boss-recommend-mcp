...
class RecommendSearchCli {
  ...
  async evaluate(expression) {
    ...
    for (let attempt = 0; attempt < 4; attempt += 1) {
      try {
        ...
        const transient = /navigated|closed|context.*destroyed|Cannot find context|Execution context/i.test(message);
  ...
  async dismissFeatureGuideCard() {
    for (let attempt = 0; attempt < 3; attempt += 1) {
      const result = await this.evaluate(`(() => {
        ...
        const doc = frame.contentDocument;
        const card = doc.querySelector('.card-feature-wrapper');
        if (!card) {
          return { ok: true, dismissed: false, present: false };
        }
        const close = card.querySelector('.card-feature-close');
        if (!close) {
          return { ok: true, dismissed: false, present: true };
        }
        close.click();
        return { ok: true, dismissed: true, present: true };
      })()`);
      if (!result?.ok) {
        throw new Error(result?.error || "FEATURE_GUIDE_CHECK_FAILED");
      }
      if (!result.present) {
        return;
      }
  ...
  async ensureTopRouteJobQuery(job) {
    const jobValue = normalizeText(job?.value || "");
    if (!jobValue) return;
    for (let attempt = 0; attempt < 4; attempt += 1) {
      const state = await this.evaluate(`((jobValue) => {
        const url = new URL(window.location.href);
        const current = url.searchParams.get('encryptJobId') || '';
        const status = url.searchParams.get('status') || '0';
        const frame = document.querySelector('iframe[name="recommendFrame"]')
          || document.querySelector('iframe[src*="/web/frame/recommend/"]')
          || document.querySelector('iframe');
        const frameSrc = frame ? String(frame.getAttribute('src') || '') : '';
        const frameHref = frame ? (() => {
          try { return String(frame.contentWindow.location.href || ''); } catch { return ''; }
        })() : '';
        if (current === jobValue && (frameSrc.includes('jobid=' + jobValue) || frameHref.includes('jobid=' + jobValue))) {
          return { ok: true, ready: true, current, frameSrc, frameHref };
        }
        const nextStatus = status && status !== 'null' ? status : '0';
        if (window.iBossRoot && window.iBossRoot.router && typeof window.iBossRoot.router.replace === 'function') {
          window.iBossRoot.router.replace({
            name: 'bpc_recommend',
            query: { encryptJobId: jobValue, status: nextStatus }
          }).catch(() => {});
          return { ok: true, ready: false, navigating_to: 'router:bpc_recommend', current, frameSrc, frameHref };
        }
        url.pathname = '/web/chat/recommend';
        url.search = '';
        url.searchParams.set('encryptJobId', jobValue);
        url.searchParams.set('status', nextStatus);
        window.location.href = url.toString();
        return { ok: true, ready: false, navigating_to: url.toString(), current, frameSrc, frameHref };
      })(${JSON.stringify(jobValue)})`);
      if (state?.ready) return;
      await sleep(1600 + attempt * 500);
    }
  }

  async suppressFrameReloadFirstJob() {
    const result = await this.evaluate(`(() => {
      const frame = ${buildRecommendFrameExpression()};
      if (!frame || !frame.contentDocument) {
        return { ok: false, error: 'NO_RECOMMEND_IFRAME' };
      ...
      for (const el of doc.querySelectorAll('*')) {
        ...
        while (vm && !seen.has(vm)) {
          seen.add(vm);
          if (typeof vm.reloadFirstJob === 'function') {
            vm.reloadFirstJob = function reloadFirstJobSuppressed() {};
            patched += 1;
          }
          vm = vm.$parent;
        }
    ...
    if (!result?.ok) {
      throw new Error(result?.error || "RELOAD_FIRST_JOB_PATCH_FAILED");
    }
  }
  ...
  async clickOptionLikeHuman(groupClass, label) {
    let option = await this.getOptionInfo(groupClass, label);
    if (!option?.ok && option?.error === "GROUP_NOT_FOUND") {
      await this.openFilterPanel();
      const ensure = await this.ensureGroupReady(groupClass);
  ...
  async selectSchoolFilter(labels) {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      if (state.defaultActive) {
        await this.clickOptionLikeHuman("school", "不限");
        await sleep(humanDelay(180, 50));
      }
  ...
  async selectDegreeFilter(labels) {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      if (state.defaultActive) {
        await this.clickOptionLikeHuman("degree", "不限");
        await sleep(humanDelay(180, 50));
      }
      for (const label of desired) {
  ...
  async run() {
    ...
    try {
      ...
      await this.dismissFeatureGuideCard();

      let selectedJob = null;
      if (this.args.job) {
        selectedJob = await this.selectJob(this.args.job);
        await this.ensureTopRouteJobQuery(selectedJob);
        await this.suppressFrameReloadFirstJob();
